...
const ReviewPage = observer(() => {
  ...
  useEffect(() => review.setReviews(book.current.reviews), [])

  const createSubmitHandler = () => {
    if (content && user.isAuth) {
      reviewApi
        .create(book.current.id, user.user.email, content)
        .then((response) => {
  ...
  return (
    <div className="content">
      <div className="card">
        ...
        <h4>{book.current.author.name}</h4>
        <p>
          <b>{book.current.publisher.name}</b>, {book.current.publisher.address}
        </p>
        <p>Категория: {book.current.category.name}</p>
      </div>
